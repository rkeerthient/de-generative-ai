...
const MessageCard = ({ message, idx }: MessageCardProps) => {
  ...
  const chatStateHasContext = useChatState((s) => s.meta.context);
  ...
  const answerCitationSplit =
    chatStateHasContext && sanitizeCitations(message.text);
  const cleanAnswer =
    chatStateHasContext && answerCitationSplit && answerCitationSplit[0];
  const citationsArray =
    chatStateHasContext &&
    answerCitationSplit &&
    JSON.parse(answerCitationSplit[1]);
  const sourcesArray =
    chatStateHasContext &&
    citationsArray &&
